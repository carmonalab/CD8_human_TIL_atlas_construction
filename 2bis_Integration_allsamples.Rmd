...
```{r eval=T}
integrated.ss <- FindNeighbors(integrated.ss, dims = 1:npcs)
integrated.ss <- FindClusters(integrated.ss, resolution = 0.5)

DimPlot(integrated.ss, reduction = "umap", group.by = "seurat_clusters", label = T) + theme(aspect.ratio = 1)
table(integrated.ss$scGate_multi, integrated.ss$seurat_clusters)
table(integrated.ss$functional.cluster, integrated.ss$seurat_clusters)
```
Assign cluster names
```{r}
tab <- table(integrated.ss$seurat_clusters, integrated.ss$functional.cluster)
tab
...
integrated.ss$functional.cluster <- newlabs[integrated.ss$seurat_clusters]
table(integrated.ss$functional.cluster)

DimPlot(integrated.ss, reduction = "umap",
