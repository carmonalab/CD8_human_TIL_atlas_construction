...
d <- DimPlot(integrated.ss.2nd, group.by = "scGate_multi", cols = mycols_scGate) + 
  ggtitle("ssSTACAS (2)") +  theme(aspect.ratio = 1)
e <- DimPlot(harmony, reduction="harmony", group.by = "scGate_multi", cols = mycols_scGate) +
  ggtitle("Harmony") + theme(aspect.ratio = 1)
(a | b | c) / (d | e)
```
...
harmomy <- subset(harmony, cells=cells.keep)
```
