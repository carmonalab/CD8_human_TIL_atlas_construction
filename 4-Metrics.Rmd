...
integrated.ss.2nd.relabel <- readRDS("cache/CD8T_integrated_2nd_relabel.rds")
```


```{r fig.height=2, fig.width=5}
a <- DimPlot(merged, group.by = "scGate_multi") + NoLegend() + theme(aspect.ratio = 1)
b <- DimPlot(integrated.ss, group.by = "scGate_multi") + NoLegend() + theme(aspect.ratio = 1)
c <- DimPlot(integrated.ss.2nd, group.by = "scGate_multi") + theme(aspect.ratio = 1)
a | b | c
```
...
integrationMetricsSummary$Method <- factor(integrationMetricsSummary$Method,
                                           levels=c("Uncorrected","Semi-supervised","Semi-supervised_2step"))
