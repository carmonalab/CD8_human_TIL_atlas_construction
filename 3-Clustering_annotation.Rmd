...
```{r fig.height=5, fig.width=7}
#Source data (use local modified version, to return actual heatmap values)
source("functions.R")
m <- celltype.heatmap.local(integrated, cluster.col = 'functional.cluster',  metadata=c("SampleLabel", "Tissue"),
                  assay = "RNA", genes = genes.use.bygroup,
                  min.cells=30, ref=integrated, cluster_genes = T, palette = palettes,
                  cluster_samples = TRUE, method=cluster.method, scale = "row",
                  return.matrix = T,
...
write.csv(m, file="aux/CD8T_heatmap_matrix.csv")

#Check we get the same heatmap
#breaks = seq(-2, 2, by = 0.1)
#color <- colorRampPalette(rev(RColorBrewer::brewer.pal(n = 7, name = "RdBu")))(length(breaks))
#pheatmap::pheatmap(m, cluster_rows = TRUE, cluster_cols = TRUE, scale = "none", show_colnames = FALSE,
#                   breaks = breaks, color = color, border_color = NA, clustering_method = "ward.D2")
...
DefaultAssay(tmp) <- "RNA"
p <- DotPlot(tmp, features = genes, cols="RdBu", scale=T, col.max=1.5) +
...
#Save source data
write.csv(x=p$data, file="aux/CD8T_dotplot_source.csv")
```
...
write.csv(x=tab.norm, file="aux/CD8T_seed_composition.csv")
