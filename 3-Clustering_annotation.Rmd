...
#                                 min.cells.group=30, min.pct=0.3,
#                                 max.cells.per.ident = 2000, logfc.threshold = 0.2,
#                                 features = NULL, test.use = "wilcox",
#                                 min.diff.pct=0.1, min.freq = 3/4, verbose = F, random.seed = 1, return.thresh=0.05)

marker.list <- ProjecTILs::FindAllMarkers.bygroup(integrated, split.by = "SampleLabel",
                                 min.cells.group=30, min.pct=0.2,
                                 max.cells.per.ident = 1000, 
                                 features = NULL, test.use = "wilcox",
                                 min.diff.pct=0.1, min.freq = 0.8, verbose = F, logfc.threshold = 0.2) #relax criteria except min.freq by group
...
# Alternative approach to find marker genes, using a cluster tree and calculating DEG hierarchically, calculated in each dataset/batch separately and ranking by occurrence

```{r}

FindAllMarkers.bygroup.byNode <- function (object, split.by = NULL, only.pos = FALSE, min.cells.group = 10, node=node, ...)  
{
    if (is.null(split.by)) {
    ...
    deg <- lapply(obj.list, function(x) {
        FindAllMarkers(x, only.pos = only.pos, min.cells.group = min.cells.group, node=node,
            ...)
        
    })
    deg
    ids <- unique(unlist(lapply(deg,function(x) x$cluster)))
...
```{r}
node.markers <- FindAllMarkers.bygroup.byNode(integrated, split.by = "SampleLabel", node=8, min.diff.pct = 0.1, min.cells.group=30, max.cells.per.ident = 1000, min.pct=0.3, logfc.threshold = 0.25)
node.markers

node.genes <- lapply(node.markers, function(x) {
  names(x) |> head(5)
})
