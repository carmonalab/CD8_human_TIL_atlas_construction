...
marker.list <- FindAllMarkers.bygroup(integrated, split.by = "SampleLabel",
                                 min.cells.group=30, min.pct=0.1,
                                 max.cells.per.ident = 1000,
                                 features = NULL, # integrated@assays$integrated@var.features,
                                 min.diff.pct=-Inf, min.freq = 0.8)

genes <- lapply(marker.list, function(x) {
  names(x) |> head(10)
})

genes.use <- unique(unlist(genes))
```
